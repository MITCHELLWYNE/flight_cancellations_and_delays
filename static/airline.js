...
d3.json(url).then(function(data) {
    ...
    function init() {
      const airlines = [...new Set(myData.map(d => d.carrier_name))].sort();
      populateDropdown("selAirline", airlines);
  
      const defaultAirline = airlines[0];
  
      const defaultData = myData.filter(d => d.carrier_name === defaultAirline);
    ...
    function createTable(data) {
      ...
      const airportHeader = headerRow.insertCell();
      airportHeader.textContent = "Airport";
      airportHeader.style.border = "1px solid black";
      airportHeader.classList.add("bold");
      ...
      processedData.forEach(function(d) {
        ...
        airlineCell.textContent = d.airport;
